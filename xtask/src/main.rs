    ...
    fn nested_contract_gate_requires_complete_architectural_and_cleanup_evidence() {
        ...
        for backend in ["direct-vmx", "outer-kvm"] {
            ...
            for broken in [
                ...
                msr.replace(
                    " guest_fields=4 guest_writes=17 guest_reject=4 guest_resume=1 guest_repeat=8 guest_operand_faults=8",
                    "",
                ),
                msr.replace("guest_fields=4", "guest_fields=3"),
                msr.replace("guest_writes=17", "guest_writes=16"),
                msr.replace("guest_reject=4", "guest_reject=3"),
                msr.replace("guest_resume=1", "guest_resume=0"),
                msr.replace("guest_repeat=8", "guest_repeat=7"),
                msr.replace("guest_operand_faults=8", "guest_operand_faults=7"),
                msr.replace(ept, ""),
